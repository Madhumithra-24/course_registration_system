...
function Forms() {
  ...
  const fetchElectiveCourseOptions = async (semester, encodedDepartment) => {
    try {
      const response = await fetch(
        `http://localhost:5555/electivecourseoption/${semester}/${encodedDepartment}`
      );
      if (response.ok) {
  ...
  const fetchOpenElectiveCourseOptions = async (
    semester,
    encodedDepartment
  ) => {
    try {
      const response = await fetch(
        `http://localhost:5555/openelectiveoption/${semester}/${encodedDepartment}`
      );
      if (response.ok) {
        const data = await response.json();
        const courseNames = data.map((course) => course.course_name);
  ...
  const fetchAddonCourseOptions = async (semester, encodedDepartment) => {
    try {
      const response = await fetch(
        `http://localhost:5555/addoncourseoption/${semester}/${encodedDepartment}`
      );
      if (response.ok) {
        const data = await response.json();
        const courseNames = data.map((course) => course.course_name);
